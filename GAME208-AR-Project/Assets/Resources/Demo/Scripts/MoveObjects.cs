...
public class MoveObjects : MonoBehaviour
{
    ...
    private Rigidbody toDragRididbody;
    ...
    void FixedUpdate()
    {
        if (Input.touchCount != 1)
        {
            ...
            if (toDragRididbody)
            {
                SetFreeProperties(toDragRididbody);
            }
            return;
        }
        ...
        if (touch.phase == TouchPhase.Began)
        {
            ...
            if (Physics.Raycast(ray, out hit) && hit.collider.tag == draggingTag)
            {
                ...
                toDragRididbody = toDrag.GetComponent<Rigidbody>();

                //calculate backwards from world point back to screen point from previous position
                dis = cam.WorldToScreenPoint(previousPosition);
                posX = Input.GetTouch(0).position.x - dis.x;
                posY = Input.GetTouch(0).position.y - dis.y;

                SetDraggingProperties(toDragRididbody);
                touched = true;
            }
        ...
        if (touched && touch.phase == TouchPhase.Moved)
        {
            ...
            toDragRididbody.velocity = worldPos / (Time.deltaTime * 10);
            previousPosition = toDrag.position;
